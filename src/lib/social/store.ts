  ...
  return {
    ...
    commitment(projectId: string, uid: string, type: CommitmentType, active: boolean, city?: string) {
      return db.runTransaction(async tx => {
        const { projectRef, project } = await projectAndPathway(tx, db, projectId);
        if (type === "bring_to_city" && active && !city?.trim()) throw new SocialError("city_required", "A city is required for this commitment.");
        const ref = db.collection("commitments").doc(`${projectId}_${uid}_${type}`);
        ...
        tx.set(ref, { projectId, uid, type, active, ...activityLabel, ...(normalized ? { city: normalized } : {}), updatedAt: FieldValue.serverTimestamp(), ...(old?.createdAt ? {} : { createdAt: FieldValue.serverTimestamp() }) }, { merge: true });
