...
export async function analyzeAnyTrailerVideo(
  ...
  const criticModel = process.env.AUDIENCE_TAKE_CRITIC_MODEL || "gemini-2.5-flash";
...
`;
  ...
  if (!criticData) {
    criticData = {
      summary: `A high-tension visual and acoustic study for "${resolvedTitle}" that uses restrained framing and tactile sonic textures to command audience focus.`,
      ...
      limitations: `Multimodal critique generated by ${criticModel} Video Engine sampling available stream frames.`
    };
  }
